...
2. `.cards`：
   - 删除 `margin-top: 2rem`（flex `gap` 已经在 `.stage` 里设了 `1.2rem`，无需额外 margin）
   - 加 `width: 100%`（因为 `.cards` 现在是 `.stage` 的 flex 子项；不显式给宽度会让 grid 容器收缩到自身最小内容宽度 `minmax(64, 96) × 1 = 64px`，所有卡片退化成纵向一列）
   - 加 `max-width: 720px`：让快捷入口块像 Chrome 默认新标签页那样成为「居中一个操作块」，由 `.stage` 的 `align-items: center` 居中
   - `justify-content: center` → `justify-content: start`：块内快捷按钮左对齐（不再水平居中）
